...
mod linux_impl {
    ...
    impl<'cl> CuTask<'cl> for V4lAutoCam {
        ...
        fn process(&mut self, clock: &RobotClock, input: Self::Input, output: Self::Output) -> CuResult<()> {
            ...
            // 2. If pci-0000:35:00.4-usb-0:1:1.3 stream is available, grab the next frame (non-blocking behaviour is
            //    controlled by the timeout we configured when opening the stream).
                ...
                if let Ok((handle, meta)) = frame {
                    ...
                    if meta.bytesused != 0 {
                        if let Some(ref fmt) = self.settled_format {
                            ...
                            debug!("V4L: No settled format, cannot process frame");
                            return Err("V4L stream format not negotiated yet".into());
                        }
                    } else {
            ...
            if let Some(last_frame_time) = self.last_frame_time && last_frame_time.elapsed().as_secs() > 1 {
                debug!("V4L: No frames received in the last second, stopping stream");
                self.stream = None; // reset stream to allow re-opening
                self.settled_format = None; // reset format as well

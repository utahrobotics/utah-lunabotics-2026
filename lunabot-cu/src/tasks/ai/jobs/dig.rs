...
pub fn dig_job() -> Job<ActuatorCommand, ()> {
    ...
    Job::spawn(
        async move {
            ...
            let _ = output_tx
                .send(ActuatorCommand::set_speed(1.0, Actuator::Bucket, Direction::Forward))
                .await;
            let _ = output_tx
                .send(ActuatorCommand::set_speed(1.0, Actuator::Lift, Direction::Backward))
                .await;
            tokio::time::sleep(std::time::Duration::from_secs_f32(0.5)).await;

            let _ = output_tx
                .send(ActuatorCommand::set_speed(0.0, Actuator::Bucket, Direction::Backward))
                .await;
            let _ = output_tx
                .send(ActuatorCommand::set_speed(0.0, Actuator::Lift, Direction::Forward))
                .await;
            tokio::time::sleep(std::time::Duration::from_secs_f32(0.5)).await;

            let _ = output_tx
                .send(ActuatorCommand::set_speed(1.0, Actuator::Lift, Direction::Forward))
                .await;
            let _ = output_tx
                .send(ActuatorCommand::set_speed(0.0, Actuator::Bucket, Direction::Backward))
                .await;

...
impl V3PicoTask {
    async fn v3pico_task(&mut self) {
        ...
        let from_pico_tx = self.from_pico_tx.clone();
        ...
        loop {
            ...
            let Ok(cmd) = cmd_result else {
                if cmd_result.err().unwrap() == RecvTimeoutError::Disconnected {
                    error!("Actuator command thread channel closed");
                }
                continue;
            };
            if let Err(e) = writer.write_all(&cmd.serialize()).await {
                error!("Failed to write to actuator port {}", e.to_string());
                break;
            }
            if let Err(e) = writer.flush().await {
                error!("Failed to flush to actuator port {}", e.to_string());
                break;
            }
        }
    }

...
pub fn get_and_init_cubecl() -> WgpuDevice {
    let setup_shared = create_wgpu_setup_from_raw();
    let device_cubecl = cubecl::wgpu::init_device(setup_shared.clone(), Default::default());
    device_cubecl
}

fn create_wgpu_setup_from_raw() -> WgpuSetup {
    cubecl::future::block_on(create_wgpu_setup_from_raw_async())
}

async fn create_wgpu_setup_from_raw_async() -> WgpuSetup {
...
fn assert_wgpu_device_existing(device: &WgpuDevice) {
    assert!(

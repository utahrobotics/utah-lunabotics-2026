...
use wgsl_pcl::{
    benchmark::{BenchmarkConfig, BenchmarkableComputePipeline},
    gpu_types::AlignedMatrix4,
    pipelines::depth_to_obstacle::{DepthToPclAndHeightPipeline, DepthToPclBenchmarkInput},
...
fn main() -> anyhow::Result<()> {
    ...
    let configs: Vec<BenchmarkConfig> = workgroup_sizes
        .into_iter()
        .map(|ws| BenchmarkConfig::new(ws).with_warmup(10).with_iterations(50))
        .collect();

    println!(
        "Benchmarking {} configurations (10 warmup, 50 timed iterations)\n",
        configs.len()
    );
    ...
    let mut results_with_cv: Vec<_> = suite
        .results
        .iter()
        .map(|r| {

use std::{
    fmt::Debug, num::FpCategory, ops::{MulAssign, SubAssign}
};
...
impl<R, C, S> MatrixOps<R, C, S> for Matrix<f64, R, C, S>
where
    R: Dim,
    C: Dim,
    S: Storage<f64, R, C>,
...
pub trait MatrixVectorOps<'a, T, R, C, S>
where
    R: Dim,
    C: Dim,
    T: std::clone::Clone + PartialEq + Debug + 'static,
    DefaultAllocator: Allocator<R, C>,
{
...
impl<'a, T, R, C, S> MatrixVectorOps<'a, T, R, C, S> for Matrix<T, R, C, S>
where
    T: Scalar,
    R: Dim,
    C: Dim,

class_name DumperShakeActuatorCommand
extends Command
...
func execute(actor: Node) -> void:
	if actor.has_method("send_shake_dumper_actuators"):
		...
		actor.send_shake_dumper_actuators(speed, frequency)
		actor.send_shake_dumper_actuators(speed)
	else:
		if not _warned_missing_method:
			_warned_missing_method = true
			push_warning("Actor does not have send_shake_dumper_actuators method")
